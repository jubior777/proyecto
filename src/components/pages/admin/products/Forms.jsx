...
const Forms = () => {
    ...
    useEffect(() => {
        if (params.id) {
            axios.get(`${API_URL}/admin/products/${params.id}`, {
    ...
    const handleSubmit = (e) => {
        ...
        const request = params.id 
            ? axios.put(`${API_URL}/admin/products/${params.id}`, body, { headers: { Authorization: `Bearer ${token()}` } }) 
            : axios.post(`${API_URL}/admin/products`, body, { headers: { Authorization: `Bearer ${token()}` } });

        request
            .then(() => nav("/admin/productos"))
            .catch((err) => {
                console.error("Error al guardar el producto:", err);
                setError(`Error al ${params.id ? "actualizar" : "crear"} el producto`);
            })
                ...
                <form onSubmit={handleSubmit}>
                    <div className="grid grid-cols-2 gap-6 mb-6">
                        <div className="col mb-4">
                            ...
                            <input type="text" name="productName" required defaultValue={product?.product_name || ''} />
                        </div>
                        <div>
                        ...
                        <div>
                            <label htmlFor="image">Imagen 1</label>
                            <input type="text" name="image" required defaultValue={product?.images?.[0] || ''} />
                        </div>
                        <div className="flex flex-col">
                            <label htmlFor="categoria">Categoría</label>
                            <select name="categoria" required defaultValue={product?.category || ''}>
                        ...
                        <div>
                            ...
                            <div className="flex gap-2">
                                <label className="flex items-center gap-1">
                                    <span>Si</span>
                                    <input type="radio" name="delivery" onChange={() => setHasDelivery(true)} checked={hasDelivery} />
                                </label>
                                <label className="flex items-center gap-1">
                                    <span>No</span>
                                    <input type="radio" name="delivery" onChange={() => setHasDelivery(false)} checked={!hasDelivery} />
                                </label>
                            </div>
                        </div>
                    </div>
